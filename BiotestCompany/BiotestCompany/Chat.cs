...
namespace BiotestCompany
{
    public class Chat
    {
        ...
        private List<User> participantList; // THIS REPLACES THE CHATPARTS TABLE

        public Chat(string name, string description, User creator, Boolean isNew) //ADD CREATOR TO PARAMETERS!!! 
        { // DO WE NEED THIS? LETS HAVE ONLY ONE CONSTRUCTOR PER CLASS
            this.chatID = GIVEME();
            this.name = name;
        ...
        public Chat(int chatID, string name, DateTime openingDT, string description, User creator, Boolean isNew) //ADD CREATOR TO PARAMETERS!!!
        {
            ...
            if (isNew)
            {
                this.participantList = new List<User>();
                this.createChat();
                Program.Chats.Add(this);
            }
            // ELSE, load users into the list from the DB
        }
        public int GIVEME()
        {
