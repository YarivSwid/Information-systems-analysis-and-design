...
namespace BiotestCompany
{
    public partial class CRUDsuppliersForm : Form
    {
        ...
        private void add_Butt_Click(object sender, EventArgs e)
        {
            try
            {
                Supplier S = new Supplier(Program.assignSuppID(), CompanyName_txt.Text, Country_txt.Text, Email_txt.Text, 0, 0, 0, "no notes yet, you can add some", new List<Contact>(), true);
            }
            catch (Exception ex)
            {
                MessageBox.Show("Please fill all fields, in order to apply changes", "Error!",
